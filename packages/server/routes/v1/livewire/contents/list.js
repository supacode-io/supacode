...
const listContentsByCollection = async ({ project = {}, params = {} }) => {
	const { id: project_id } = project;
	const { collection_code, locale_code } = params;
	...
	const contents = await queryListContent(sql, { collection_id: collection.id, locale_id: locale.id });

	return { contents };
};

const routes = async (fastify) => {
	fastify.get('/v1/:project_code/livewire/contents/:collection_code', async (request, reply) => {
		const { project, params, query } = request;
		const result = await listContentsByCollection({ project, params: { ...params, ...query } });
		return reply.status(200).send(result);
	});
};

module.exports = { default: routes, listContentsByCollection };
